...
Drone::Drone(uint32_t id, Ptr<Node> node) : m_id(id), m_node(node) {
  if (!m_node) {
    return;
  }
  ...
  if (m_radio.socket) {
    m_radio.socket->SetRecvCallback(MakeCallback(&Drone::SocketRx, this));
  }
}


void Drone::Start() {
  Simulator::Schedule(Seconds(0.0), MakeCallback(&Drone::OnTick, this));
}
...
void Drone::OnTick() {
  ...
  Simulator::Schedule(Seconds(m_dt), MakeCallback(&Drone::OnTick, this));
}


void Drone::SocketRx(Ptr<Socket> sock) {
  Address from;
  Ptr<Packet> p;
...
void Drone::SendTo(Ipv4Address dst, Ptr<Packet> p) {
  if (!m_radio.socket || !p) {
    return;
  }
...
void Drone::SendBroadcast(Ptr<Packet> p) {
  if (!m_radio.socket || !p) {
    return;
  }
...
void Drone::OnReceive(Ptr<Packet> p) {}