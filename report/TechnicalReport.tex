...
reorganize themselves to restore connectivity. This is achieved through local
communication and decentralized control. \\
Specifically, after the loss of communication between a
...
formation control laws required to perform tasks effectively~\cite{ouyang2023formation}. \\
...
mirrors the Crazyflie~2.1 ecosystem~\cite{bitcraze_crazyflie} as closely as possible. In particular, it
...
through the ACK-based reachability mechanism during periodic position reporting
...
virtual spring-damper model~\cite{piemngam2019virtual, chen2015virtual}. The controller runs periodically and produces a
...
them.
\begin{equation} 
    {F}_{tot} = F_{tot} + K_{att} \cdot (\vec{p}_j - \vec{p}_i) \quad \forall j\in \{N_{prev} \cup N_{next}\}
\end{equation}
...
\begin{gather}
    \vec{F}_{tot} = \vec{F}_{tot} - K_{rep} \cdot \left(\frac{1}{dist^2}\right) \cdot \frac{\vec{p}_j - \vec{p}_i}{dist} \quad \forall j \in \{N \mid dist < D_{safe}\} \\
    \text{where } dist = \|\vec{p}_i - \vec{p}_j\|
\end{gather}
...
scenario implemented in \textit{ns-3}~\cite{ns3simulator}, a discrete-event network simulator widely
...
three drones ($D_1$, $D_2$, $D_3$). The initial positions are chosen to maintain all
the drones inside the coverage area of the base station.  
The base station is placed at the origin $(0,0,0)$ while the drones are located
at $(40,15,0)$, $(60,20,0)$ and $(30,25,0)$. \\
The base station is responsible for triggering flooding to enable hop discovery:
in this simulation, our base station schedules a \texttt{START} flood message
with drone $D_1$ as initiator every 0.5 \, s. \\
Regarding drones, each one sets its periodic tick at 0.05 \, s: each tick implies
...
\end{itemize}
To avoid drift accumulation, each next tick event is scheduled exactly 0.05
...
mission mode active, remaining in the idle state as virtual anchor: this design 
choice prevents the relay chain from collapsing due to the requester suffering 
only attractive forces from preceding neighbors, since it represents the 
estremity of the chain. For this reason, the self-healing protocol is driven by
drones that hear the request. \\
...
\begin{itemize}
    \item mass = 0.029 \, kg;
    \item maximum velocity $V_{max}$ = 1 \, m/s.
\end{itemize}
...
models, which can be subsequently rendered using the \textit{NetAnim}~\cite{netanim}
visualization tool. 
...
Max drone speed & $V_{max}$ & Max speed (the exact same of the CrazyFlie 2.1) \\ \hline
...
Max drone speed & $1$ m/s \\ \hline
Attractive gain & $1$ \\ \hline
Repulsive gain & $8$ \\ \hline
Safety distance & $2$  \\ \hline
Node tick distance & $0.05$ s \\ \hline
Simulation duration & $150$ s \\ \hline
\end{tabular}
\end{table}
...
Average First Oscillation & $0.09$ m \\ \hline
Average Oscillation & $0.0944$ m \\ \hline
Average Last Oscillation & $0.098$ m \\ \hline
Average Min Distance & $1.46$ m \\ \hline
Average Time to Target & $13.12$ s \\ \hline
Connection Restoration Time & $1.669$ s \\ \hline
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
    ...
    \caption{Nodes connections over the first 5 seconds}
    \label{fig:connections_over_time}
\end{figure}
Figure~\ref{fig:connections_over_time} shows the number of active 
connections for each node during the first five seconds of the simulation. 
At simulation  start, the base station maintains direct connectivity 
with drones $D_1$ and $D_3$, while $D_2$ remains disconnected due to its
position outside the coverage area. \\
After the ACK timeout expires, $D_2$ broadcasts its \texttt{HELP\_PROXY} 
request at $t \approx 1.5$\,s, triggering the relay mechanism. The base 
station connectivity is restored at $t=1.58$\,s, when the first relayed 
\texttt{PositionUpdate} from $D_2$ reaches it through a helper drone. 
Shortly after, at $t=1.67$\,s, the lost drone $D_2$ receives its first 
relayed \texttt{PositionAck}, confirming end-to-end connectivity. \\
It is worth noting that these restoration times remain constant across
different parameter configurations because of the \textit{Crazyflie} mirrored
architecture. In fact, since the relaying drone is always within communication
range of both the base station and the lost drone, the relay path is 
established immediately after the \texttt{HELP\_PROXY} broadcast, regardless
of the controller gains or safety distance settings. \\
...
elapsed simulation time between actuation calls. This design choice ensures
full deterministic reproducibility across runs: each drone position is computed 
as a function of commanded accelerations and discrete time steps. Furthermore, 
the mobility model implements a simplified kinematic integration that neglects 
aerodynamic effects, environmental disturbances, and actuator dynamics. 
Such abstractions may lead to an optimistic estimation of the controller's 
effectiveness compared to real-world conditions. \\ 
