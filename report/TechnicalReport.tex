...
them.
\begin{equation} 
    {F}_{tot} = F_{tot} + K_{att} \cdot (\vec{p}_j - \vec{p}_i) 
    \quad \forall j\in \{N_{prev} \cup N_{next}\} 
\end{equation}
...
\begin{gather}
    \vec{F}_{tot} = \vec{F}_{tot} - K_{rep} \cdot \left(\frac{1}{dist^2}\right) 
    \cdot \frac{\vec{p}_j - \vec{p}_i}{dist} \quad \forall j \in \{N \mid dist < D_{safe}\} \\
...
elapsed simulation time between actuation calls. While this is sufficient for a 
qualitative evaluation, it prevents full deterministic reproducibility when
the simulation is run with different random seeds. Furthermore, the mobility model
